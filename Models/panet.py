...
class predictClass(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(nn.Conv2d(in_channels=3, out_channels=32, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(32), nn.ReLU(), nn.MaxPool2d(kernel_size=2, padding=0), nn.Conv2d(
                in_channels=32, out_channels=16, kernel_size=3, stride=1, padding=1), nn.BatchNorm2d(16), nn.ReLU(),
                                     nn.MaxPool2d(kernel_size=2, padding=0),
                                     nn.Conv2d(in_channels=16, out_channels=8, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(8), nn.ReLU())

        self.classifier = nn.Sequential(nn.Linear(in_features=128 * 128 * 8, out_features=1024), nn.ReLU(),
                                        nn.Linear(in_features=1024, out_features=16), nn.ReLU(),
...
class IOULoss(nn.modules.loss._Loss):
    ...
    def forward(self, bbox, bbox_pred):
        ...
        area_intersection = (torch.min(bbox[:, 0, 2], bbox_pred[:, 0, 2]) - torch.max(bbox[:, 0, 0],
                                                                                      bbox_pred[:, 0, 0])) * (
                                    torch.min(bbox[:, 0, 3], bbox_pred[:, 0, 3]) - torch.max(bbox[:, 0, 1],
                                                                                             bbox_pred[:, 0, 1]))
